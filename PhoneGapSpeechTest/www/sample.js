...
function doInit() {  
    var serverURL = prompt("Please supply a server URL");
...
function printTTSResult(resultObject) {
    ...
    if (resultObject.eventName != undefined) {
        if (resultObject.eventName == 'TTSStarted' || resultObject.eventName == 'TTSPlaying') {
            //speechKit.queryNextEvent( function(r){printTTSResult(r)}, function(e){printTTSResult(e)} );
        }
        if (resultObject.eventName == 'TTSStopped' || resultObject.eventName == 'TTSComplete') {
            var playButton = document.getElementById("play-tts");
...
function printRecoResult(resultObject) {
    if (resultObject.eventName == 'RecoVolumeUpdate') {
        setVolumeLevel(resultObject);
    }
    else {
        if (resultObject.eventName == 'RecoComplete' || resultObject.eventName == 'RecoStopped') {
            var stopButton = document.getElementById("stop-reco");
            stopButton.disabled = true;
...
function getHtml(resultObject) {
    ...
    if (resultObject.eventName != undefined) {
        htmlText = htmlText + "<ul><li>Event: " + resultObject.eventName;
        htmlText = htmlText + "</li></ul>";
    }
    return htmlText;
}
